...
int GDPL_kontroller_les_fra_fil()
{
    ...
    if (file == 0) {
        ...
        if (sz == 0) {
            ...
            /* Fila eksisterer, men er tom. Dette er feil! */
        ...
        GDPL_log(GDPL_DEBUG, signatur, "TODO: les inn ");
        ...
        GDPL_konkurranse_data_node n;

        size_t size = fread(&n,sizeof(GDPL_konkurranse_data_node),1,file);
...
int GDPL_kontroller_skriv_til_fil()
{
    ...
    /* Skriv root-noden til fila. */

    GDPL_konkurranse_data_node* p = gdpl_kontroller_konkurranseliste_root_ptr;
    size_t sz = sizeof(GDPL_konkurranse_data_node);
